...
impl Ext2 {
    ...
    pub fn open(block_device: Arc<dyn BlockDevice>) -> Result<Arc<Self>> {
        ...
        let group_descriptors_segment = {
            let num_pages = ((super_block.block_groups_count() as usize)
                * core::mem::size_of::<RawGroupDescriptor>())
            .div_ceil(BLOCK_SIZE);
            let segment = VmAllocOptions::new(num_pages)
                .uninit(true)
                .is_contiguous(true)
                .alloc_contiguous()?;
    ...
    pub fn sync_metadata(&self) -> Result<()> {
        ...
        let mut bio_complete = BioComplete::new();
        ...
        bio_complete.concat(self.block_device.write_blocks(
        ...
        for idx in 1..super_block.block_groups_count() {
            if super_block.is_backup_group(idx as usize) {
                ...
                bio_complete.concat(self.block_device.write_bytes_async(
        ...
        for bio_status in bio_complete.wait() {
            match bio_status {
                BioStatus::Complete => {}
                err_status => {
                    return Err(Error::from(err_status));
