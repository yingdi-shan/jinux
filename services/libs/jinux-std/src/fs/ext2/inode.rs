...
impl InodeImpl_ {
    pub fn new(desc: Dirty<InodeDesc>, weak_self: Weak<Inode>) -> Self {
        Self {
            blocks_hole_desc: BlocksHoleDesc::new(desc.blocks_count()),
            desc,
            is_freed: false,
            weak_self,
        }
    ...
    pub fn read_block(&self, bid: Bid, block: &VmFrame) -> Result<()> {
        ...
        if self.blocks_hole_desc.is_hole(bid) {
            block.zero();
            return Ok(());
        }
    ...
    pub fn resize(&mut self, new_size: usize) -> Result<()> {
        ...
        self.blocks_hole_desc.resize(new_blocks);
        Ok(())
    }
}
...
impl InodeImpl {
    ...
    pub fn write_block(&self, bid: Bid, block: &VmFrame) -> Result<()> {
        ...
        if inner.blocks_hole_desc.is_hole(bid) {
            drop(inner);
            let mut inner = self.0.write();
            if (bid as usize) < inner.blocks_hole_desc.size() && inner.blocks_hole_desc.is_hole(bid)
            {
