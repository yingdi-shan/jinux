...
use jinux_block::{
    bio::{BioEnqueueError, BioStatus, BioType, SubmitBio},
    id::Sid,
...
impl BlockDevice {
    ...
    pub(crate) fn init(transport: Box<dyn VirtioTransport>) -> Result<(), VirtioDeviceError> {
        ...
        jinux_block::register_device(super::DEVICE_NAME.to_string(), Arc::new(block_device));
        Ok(())
    }
...
impl BioRequestQueue for BioSingleQueue {
    ...
    ///
    /// When enqueueing the `SubmitBio`, try to insert it into the last request if the
    /// type is same and the sector range is contiguous.
    /// Otherwise, creates and inserts a new request for the `SubmitBio`.
