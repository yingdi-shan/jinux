...
use super::{
    bio::{general_complete_fn, Bio, BioComplete, BioEnqueueError, BioSegment, BioStatus, BioType},
    id::{Bid, Sid},
    BlockDevice, BLOCK_SIZE, SECTOR_SIZE,
};

use jinux_frame::vm::{VmAllocOptions, VmFrame, VmIo, VmSegment};
...
impl dyn BlockDevice {
    ...
    pub fn read_blocks(
        ...
        bid: Bid,
        segment: &VmSegment,
    ) -> Result<BioComplete, BioEnqueueError> {
        let bio = create_bio_from_segment(BioType::Read, bid, segment);
        bio.submit(self)
    }
    ...
    pub fn read_block(&self, bid: Bid, frame: &VmFrame) -> Result<BioComplete, BioEnqueueError> {
        let bio = create_bio_from_frame(BioType::Read, bid, frame);
        bio.submit(self)
    }
    ...
    pub fn write_block(&self, bid: Bid, frame: &VmFrame) -> Result<BioComplete, BioEnqueueError> {
        let bio = create_bio_from_frame(BioType::Write, bid, frame);
        bio.submit(self)
    }
}
...
impl VmIo for dyn BlockDevice {
    ...
    fn read_bytes(&self, offset: usize, buf: &mut [u8]) -> jinux_frame::Result<()> {
        ...
        match status {
            ...
            _ => Err(jinux_frame::Error::IoError),
        }
    }
...
impl dyn BlockDevice {
    ...
    pub fn write_bytes_async(&self, offset: usize, buf: &[u8]) -> jinux_frame::Result<BioComplete> {
        if offset % SECTOR_SIZE != 0 || buf.len() % SECTOR_SIZE != 0 {
            return Err(jinux_frame::Error::InvalidArgs);
        }
        if buf.is_empty() {
            return Ok(BioComplete::new());
        }
